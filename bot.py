...
def generate_plot_for_telegram(params, single_point_coords):
    ...
    ax.legend()
    ax.set_title(f'Concentration Distribution at {plot_height_z}m Height')
...
async def get_y(update: Update, context: ContextTypes.DEFAULT_TYPE):
    context.user_data['current_state'] = GET_Y
    return await get_numeric_input(update, context, 'y', "۳. ارتفاع گیرنده از سطح زمین (z) به متر:", GET_Z)
async def get_z(update: Update, context: ContextTypes.DEFAULT_TYPE):
    context.user_data['current_state'] = GET_Z
    return await get_numeric_input(update, context, 'z', "۴. نرخ انتشار آلاینده (Q) به گرم بر ثانیه:", GET_Q)
async def get_q(update: Update, context: ContextTypes.DEFAULT_TYPE):
    context.user_data['current_state'] = GET_Q
    return await get_numeric_input(update, context, 'Q_emission', "۵. سرعت باد مرجع (u_ref) به متر بر ثانیه:", GET_U_REF)
async def get_u_ref(update: Update, context: ContextTypes.DEFAULT_TYPE):
    context.user_data['current_state'] = GET_U_REF
    return await get_numeric_input(update, context, 'u_ref', "۶. ارتفاع مرجع (z_ref) به متر:", GET_Z_REF)
async def get_z_ref(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        await update.message.reply_text("ورودی نامعتبر است. لطفاً یک عدد صحیح وارد کنید.")
        return GET_Z_REF
async def get_stability(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_input = update.message.text.upper()
    if user_input in ['A', 'B', 'C', 'D', 'E', 'F']:
        context.user_data['stability_class'] = user_input
        reply_keyboard = [['urban', 'rural']]
        await update.message.reply_text("۸. نوع منطقه را انتخاب کنید:", reply_markup=ReplyKeyboardMarkup(reply_keyboard, one_time_keyboard=True))
        return GET_AREA
    else:
...
async def get_area(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_input in ['urban', 'rural']:
        ...
        return GET_AREA
async def get_hm(update: Update, context: ContextTypes.DEFAULT_TYPE):
    context.user_data['current_state'] = GET_HM
    return await get_numeric_input(update, context, 'Hm_boundary_layer', "۱۰. قطر داخلی دودکش (ds) به متر:", GET_DS)
async def get_ds(update: Update, context: ContextTypes.DEFAULT_TYPE):
    context.user_data['current_state'] = GET_DS
    return await get_numeric_input(update, context, 'ds_stack_diameter', "۱۱. ارتفاع فیزیکی دودکش (hs) به متر:", GET_HS)
async def get_hs(update: Update, context: ContextTypes.DEFAULT_TYPE):
    context.user_data['current_state'] = GET_HS
    return await get_numeric_input(update, context, 'hs_stack_height', "۱۲. دمای گاز خروجی (Ts) به کلوین:", GET_TS)
async def get_ts(update: Update, context: ContextTypes.DEFAULT_TYPE):
    context.user_data['current_state'] = GET_TS
    return await get_numeric_input(update, context, 'Ts_stack_temp', "۱۳. دمای هوای محیط (Ta) به کلوین:", GET_TA)
async def get_ta(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        await update.message.reply_text("ورودی نامعتبر است. لطفاً یک عدد صحیح وارد کنید.")
        return GET_TA
async def get_vs_choice(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if choice == 'vs':
        ...
        return GET_VS_CHOICE
async def get_vs(update: Update, context: ContextTypes.DEFAULT_TYPE):
    context.user_data['current_state'] = GET_VS
    return await get_numeric_input(update, context, 'vs_stack_velocity', "۱۵. و در آخر، نیمه عمر آلاینده (T1/2) به ثانیه (برای آلاینده پایدار 0 وارد کنید):", GET_HALF_LIFE)
async def get_qs(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
